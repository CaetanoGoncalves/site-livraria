...
func SearchName(w http.ResponseWriter, r *http.Request) {
	...
	searchQuery := `SELECT * FROM books WHERE name ILIKE '%' || $1 || '%'`
	rows, err := DB.Query(searchQuery, book.Name)
	if err != nil {
	...
	if len(books) == 0 {
		http.Error(w, "Nenhum livro com este nome encontrado", http.StatusNotFound)
		return
	}
